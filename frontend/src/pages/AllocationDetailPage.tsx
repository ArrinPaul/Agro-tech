    ...
    if (!allocation) {
        return (
            <div className="text-center py-20">
                ...
                <p className="text-gray-500 font-medium">Allocation not found</p>
    ...
    return (
        <div className="space-y-5">
            ...
            <div className="flex items-center gap-3">
                <button onClick={() => navigate("/allocations")} className="p-2 rounded-lg hover:bg-gray-100 text-gray-500">
                    <ArrowLeft size={18} />
                </button>
                <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="bg-white rounded-xl border border-gray-200 p-5">
                    ...
                    <dl className="space-y-3 text-sm">
                        <div className="flex justify-between">
                            ...
                            <dd className="font-medium text-gray-900">{allocation.cropName}</dd>
                        </div>
                        {crop && (
                            <>
                                <div className="flex justify-between">
                                    <dt className="text-gray-500">Status</dt>
                                    <dd className="font-medium text-gray-900">{crop.status}</dd>
                                </div>
                                <div className="flex justify-between">
                                    <dt className="text-gray-500">Total Quantity</dt>
                                    <dd className="font-medium text-gray-900">{crop.quantity.toLocaleString()} units</dd>
                                </div>
                            </>
                        )}
                    </dl>
                ...
                <div className="bg-white rounded-xl border border-gray-200 p-5">
                    ...
                    <dl className="space-y-3 text-sm">
                        <div className="flex justify-between">
                            <dt className="text-gray-500">Name</dt>
                            <dd className="font-medium text-gray-900">{allocation.warehouseName}</dd>
                        </div>
                        {warehouse && (
                            <>
                                <div className="flex justify-between">
                                    <dt className="text-gray-500">Location</dt>
                                    <dd className="font-medium text-gray-900">{warehouse.location}</dd>
                                </div>
                                <div className="flex justify-between">
                                    <dt className="text-gray-500">Current Utilization</dt>
                                    <dd className="font-medium text-gray-900">{whUtil}%</dd>
                                </div>
                            </>
                        )}
                    </dl>
            ...
            {/* Allocation details */}
            <div className="bg-white rounded-xl border border-gray-200 p-5">
                <h2 className="font-semibold text-gray-900 mb-4">Allocation Summary</h2>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div>
                        <p className="text-xs text-gray-500 mb-1">Allocated Quantity</p>
                        <p className="text-xl font-bold text-gray-900">{allocation.allocatedQuantity.toLocaleString()}</p>
                    </div>
                    <div>
                    ...
                    <div>
                        <p className="text-xs text-gray-500 mb-1 flex items-center gap-1"><User size={12} /> Created By</p>
                        <p className="text-sm font-medium text-gray-700">{allocation.createdByName}</p>
                    </div>
                    <div>
                        <p className="text-xs text-gray-500 mb-1">ID</p>
                        <p className="text-sm font-mono text-gray-500">{allocation._id}</p>
                    </div>
                </div>
            </div>
            ...
            {linkedResources.length > 0 && (
                <div className="bg-white rounded-xl border border-gray-200 p-5">
                    <h2 className="font-semibold text-gray-900 mb-4">Resources Consumed</h2>
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b border-gray-100">
                                <th className="text-left py-2 font-medium text-gray-600">Resource</th>
                                <th className="text-left py-2 font-medium text-gray-600">Per Unit</th>
                                <th className="text-left py-2 font-medium text-gray-600">Total Used</th>
                                <th className="text-left py-2 font-medium text-gray-600">Remaining Stock</th>
                            </tr>
                        </thead>
                        <tbody>
                            {linkedResources.map((lr) => (
                                <tr key={lr._id} className="border-b border-gray-50">
                                    ...
                                    <td className="py-2 font-semibold text-gray-900">{lr.consumed.toLocaleString()}</td>
                                    <td className="py-2 text-gray-600">{lr.resource?.stockQuantity?.toLocaleString() ?? "—"}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
